...
最近很多人和我反馈程序点击二维码直接就崩溃，我发给很多朋友测试都说没有这个问题，最后这个bug我只在一台纯净的W10上复现出来了。所以我理解各位的电脑上应该是没有VS开发环境。

程序崩溃的根本原因是因为dll注入失败。dll注入失败，程序自然也就无法正常运行。这种因为环境原因而产生的问题，我也没有什么好的解决方案。

最后，一个不算解决方案的解决方案：安装VS
